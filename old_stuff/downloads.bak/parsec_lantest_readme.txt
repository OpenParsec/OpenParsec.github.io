...
------------------------------------------------------------------------------
PARSEC LAN-TEST (build 0190)                                 October 3, 2000.
------------------------------------------------------------------------------
...
Additional requirements:
  Linux Kernel 2.2, glibc 2.1.x, X11 (for Voodoo1/2/3 or OpenGL)
...
Graphics card:
  ...
  * OpenGL compatible hardware accelerator with at least 8MB of VRAM
    (e.g. ATI Rage128, cards with less than 8MB VRAM are unsupported, but may 
    work)
...
Here you can select one of six spacecraft by using the left/right arrow keys
and pressing <enter>.
...
You can also toggle sound/music and joystick support on and off.
...
four) compete for the highest number of kills. As soon as the kill limit
has been reached, the player with the most kills wins. The kill limit can
be configured in the options menu.
...
(cycle targets with <t>). The small monitor mounted on the left side of the
radar will show "incoming" when enemy guided missiles have locked onto you.
...
Invisibility (always disabled) - Invulnerability - Afterburner -
Holo Device (always disabled)
...
have Device3dfx installed. See http://linux.3dfx.com for details.
...
Specifically this means NVIDIA cards (TNT, TNT2, GeForce, GeForce2), and
...
Parsec will try to load libglide2x.so. If your Glide library has a different
name (e.g. libglide.so.2), you should consider adding a symlink.
...
advised to use the latest drivers included with the OpenGL 1.1.3 release
(part of MacOS 9.0.4).
